...
// 取值范围：就是临界点包不包含等于
    ...
    public static boolean isOverIntervalLimit(String startDateStr, String endDateStr, int interval) {
        ...
        Date endDate = null;
        try {
            startDate = DateUtils.parseDate(startDateStr, DateUtil.DATE_FORMAT_DATEONLY);
            endDate = DateUtils.parseDate(endDateStr, DateUtil.DATE_FORMAT_DATEONLY);
        } catch (ParseException e) {
            logger.error(e);
            return false;
        }
    ...
    public static Date getMinTime(Date dt) {
        return DateUtil.getDateByStr(DateUtil.formatDate(dt, "yyyy-MM-dd"));
    }
